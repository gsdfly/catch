<template>
  <div class="operations" v-if="operations.length>0 && imgscurrent.length>0">
    ...
    <div v-for="(item,index) in operations.slice(0,2)">
      <!--运营位类别:优惠券-->
      <div v-if="item.type === 1">
        <!--优惠券样式类型、优惠券状态-->
        <img @click.prevent="" v-if="item.coupon.status === 0" :src="imgscurrent[index].img_receive"
             @click="consumer(index,item.coupon.id,item.coin_price.coin_price_id,item.coupon.type,item.coin_price.coin_num,item.coin_price.type)"
             id="free_doll"/>
        ...
        <span v-cloak v-if="item.coupon.type === 2 && item.coin_price.type === 0"
              class="quan_game_num">{{item.coin_price.coin_num / info.coin_num +'次' | handleN}}</span>
        ...
        <p class="infinity-tip" style="pointer-events: none;" :class="{'already-use':item.coupon.status===2}">
          <span
            v-if="item.coin_price.type === 1 && item.coupon.type===1">{{(item.coin_price.coin_price - item.coupon.reduce).toFixed(2) | handleNum}}元</span><span
      ...
      <div v-if="item.type===2" @click="openXcx(item.url,item.action)">
        ...
        <span v-if="item.pxark" class="more" @click.stop="openDetails(item.pxark)"></span>
      </div>
    </div>
  </div>
</template>
...
//      需要根据优惠券的样式类型判断
    ...
    methods: {
      ...
      //领取优惠券
      consumer(index, coupon_id, coin_price_id, coupon_type, coin, type) {
            ...
            this.$store.dispatch('getFreeCoin', {coin_price_id: coin_price_id, coupon_id: coupon_id}).then((data) => {
              ...
              for(var i=0;i<this.operations.length;i++){
                if(this.operations[i].coupon.type ===2){
                  this.operations[i].coupon.status = 1;
                  break;
                }
              }
