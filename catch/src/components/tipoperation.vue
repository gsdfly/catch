    ...
    methods: {
      ...
      tipAction() {
        ...
        switch (this.show_tip_operation.tipObj.action) {
          case "2":
            window.location.href = this.show_tip_operation.tipObj.url;
            localStorage.setItem('isLinkOut', 'true');
            break;
          case "3":
            Toast({message: this.show_tip_operation.tipObj.url, position: 'bottom'});
        }
      }
    }
  }
